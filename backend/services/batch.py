    ...
    async def _process_single_file(
        ...
        async with semaphore:
            try:
                ...
                # Save temp file to an ephemeral directory
                ...
                extracted_data = await llm_extraction_service.extract_study_data(
                    ...
                    outcome=batch_job.outcome,
                    exposure=batch_job.exposure,
                    population=batch_job.population,
                    comparison=batch_job.comparison,
                    study_design=batch_job.study_design,
                ...
                if first_author and first_author != "Unknown":
                    # Get just the first author name (split by comma, semicolon, or ' and ')
                    first_author = first_author.split(",")[0].split(";")[0].split(" and ")[0].strip()
                    # Remove trailing 'et al.' if present
                    first_author = first_author.replace("et al.", "").replace("et al", "").strip()
                    # If still empty after cleaning, fallback to Unknown
                    if not first_author:
