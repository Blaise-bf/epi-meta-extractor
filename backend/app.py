...
async def upload_pdf(
    ...
    effect_type: str = Query("OR"),
    meta_analysis_id: str = Query(None),
    outcome: str = Query(...),
    exposure: str = Query(...),
    background_tasks: BackgroundTasks = BackgroundTasks(),
    ...
    try:
        ...
        tmp_path = Path(settings.TEMP_DIR) / file.filename
        ...
        # NOTE: Effect sizes (effect_value, ci_lower, ci_upper) are optional
        # Studies can be accepted even without effect size data as long as they examine the relationship
        if not extracted_data or not extracted_data.get("analysis"):
            return {
                "status": "not_relevant",
                "message": f"Study is not relevant: No analysis data found related to outcome '{outcome}' and exposure '{exposure}'",
        ...
        # Validate that extracted outcome and exposure are related to what user requested
        # Effect sizes are not required for validation
        if not extracted_outcome or not extracted_exposure:
            return {
                "status": "not_relevant",
                "message": f"Study does not contain data about outcome '{outcome}' and exposure '{exposure}'",
