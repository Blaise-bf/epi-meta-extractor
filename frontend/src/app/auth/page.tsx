...
export default function AuthPage() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        saveAuthToStorage(access_token, user);
  ...
  const handleRequestLink = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/request-link`, { email });
      if (response.data?.magic_link) {
        setDevLink(response.data.magic_link);
      }
        ...
        <div className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm p-8 shadow-xl">
          <div className="mb-6">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              We use passwordless magic links. Enter your email and we will send a secure link to your inbox.
            </p>
          </div>
          ...
          <form onSubmit={handleRequestLink} className="space-y-4">
            ...
            <button
              ...
              className="w-full rounded-xl bg-slate-900 dark:bg-slate-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 dark:hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-70"
            >
          ...
          {devLink && (
            <div className="mt-4 rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-xs text-amber-800 dark:text-amber-400">
              <div className="font-semibold uppercase tracking-[0.12em]">Dev Link</div>
              <a
                href={devLink}
